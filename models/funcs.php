...
function fetchAllUsers()
{
	...
	$stmt = $mysqli->prepare("SELECT
		...
		FROM ".$db_table_prefix."users");
	$stmt->execute();
	$stmt->bind_result($id, $user, $password, $token, $activationRequest, $passwordRequest, $active, $title, $signUp, $signIn, $realname, $email);

	while ($stmt->fetch()){
		$row[] = array('id' => $id, 'user_name' => $user, 'password' => $password, 'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn, 'real_name' => $realname, 'email' => $email);
	}
	$stmt->close();
	return ($row);
}
...
function updateRealName($id, $realname)
{
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
		SET real_name = ?
		...
		id = ?
		LIMIT 1");
	$stmt->bind_param("si", $realname, $id);
	$result = $stmt->execute();
	$stmt->close();
	return $result;
}
